...
8. 最后必须严格按照"输出报告格式要求"章节的结构输出总结报告
...
- `get_column_data(column_name, context)` — 获取指定列所有数据（返回列表），`context` 可用于限定到当前激活 tab 等局部表格
...
- 搜索和重置类用例优先尝试复用公共 `search()`、`btn_reset`；若搜索框位于详情页 tab、自定义 header 或局部容器中，公共方法无法稳定命中时，才允许补充最小业务侧方法
- 删除类用例必须校验资源已从列表消失，优先使用 `assert_deleted()`
- 列表断言优先使用框架公共断言方法，不要重复造轮子

### 4. fixture 与数据管理

- 用例必须优先复用已有 fixture、页面对象方法、工具函数、日志方法、数据加载方式
- 多条用例存在重复前置/清理逻辑时，优先检查能否复用或扩展同目录 `conftest.py`，不要在测试函数中重复造数据
- 若 3 条以上用例共享同一类前置或清理逻辑，优先评估是否适合抽成 `class` 级别 fixture；若依赖 `function` 级页面/浏览器状态，或存在状态污染风险，可保留为 `function` 级 fixture
...
- 严禁使用 XPath
- `first()`、`nth()` 不作为首选；应先缩小到当前可见 `dialog`、当前激活 `tab`、目标表格或目标行，再在最小范围内作为兜底方案使用
...
任务完成后，必须严格按照以下结构输出总结报告，不得增删章节、调整顺序或改写标题：
...
- 是否只补充了最小必要的页面对象方法
- 是否识别 3 条以上用例存在重复的前置/清理逻辑，并优先评估是否适合抽成 class 级别的 fixture
...
### 3. 关键修复点（如有）
如果在运行过程中遇到报错并修改了代码，请列出修复的具体问题（例如：定位器超时、弹窗文案不匹配、下拉框元素遮挡等）及解决方案。
若无修复点，写 `- 无`
