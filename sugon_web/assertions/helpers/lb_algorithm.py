...
def _assert_common_lb_expectations(stats, scene_name):
    ...
    assert stats["total"] > 0, (
        f"[ScenarioAssertion] {scene_name} | 未采集到任何有效响应 | "
        f"期望: 至少1个 | 实际: 0个 | 原始结果: {stats['raw_responses']}"
    )
    assert not stats["unexpected_responses"], (
        f"[ScenarioAssertion] {scene_name} | 返回非预期内容 | "
        f"期望: 无未知响应 | 实际: {stats['unexpected_responses']} | "
        f"全量结果: {stats['raw_responses']}"
    )
    assert not stats["missing_backends"], (
        f"[ScenarioAssertion] {scene_name} | 存在未命中后端 | "
        f"期望: 全部命中 | 实际: 未命中 {stats['missing_backends']} | "
        f"统计结果: {stats['counts']}"
    )
...
def _assert_round_robin(stats, scene_name, tolerance):
    ...
    for backend_key in stats["backend_keys"]:
        ...
        assert deviation <= tolerance, (
            f"[ScenarioAssertion] {scene_name} | 轮询分布异常 | "
            f"期望: 后端 '{backend_key}' 比例约 {expected_ratio:.4f} | "
            f"实际: {actual_ratio:.4f} | 允许偏差: {tolerance:.4f} | "
...
def _assert_weighted_round_robin(stats, scene_name, weights, tolerance):
    ...
    for backend_key in stats["backend_keys"]:
        ...
        deviation = abs(actual_ratio - expected_ratio)
        assert deviation <= tolerance, (
            f"[ScenarioAssertion] {scene_name} | 加权轮询分布异常 | "
            f"期望: 后端 '{backend_key}' (权重 {weights[backend_key]}) 比例约 {expected_ratio:.4f} | "
            f"实际: {actual_ratio:.4f} | 允许偏差: {tolerance:.4f} | "
            f"统计结果: counts={stats['counts']}, total={stats['total']}"
        )
...
def assert_udp_source_ip_sticky(hit_map, scene_name, expected_count=None):
    ...
    assert not missing, (
        f"[ScenarioAssertion] {scene_name} | 存在未命中消息 | "
        f"期望: 全部命中 | 实际: 未命中 {missing} | 完整分布: {hit_map}"
    )
    hit_backends = {backend for backend in hit_map.values() if backend is not None}
    assert len(hit_backends) == 1, (
        f"[ScenarioAssertion] {scene_name} | 源 IP 算法命中多台后端 | "
        f"期望: 1台 | 实际: {hit_backends} | 完整分布: {hit_map}"
    )
    assert len(hit_map) >= expected_count, (
        f"[ScenarioAssertion] {scene_name} | 命中消息数不足 | "
        f"期望: >= {expected_count} | 实际: {len(hit_map)}"
    )
...
def assert_udp_all_rejected(hit_map, scene_name):
    ...
    assert not received, (
        f"[ScenarioAssertion] {scene_name} | 访问控制未生效 | "
        f"期望: 全部拒绝 | 实际: 以下消息被接收: {received}"
    )
