...
class WptAssertionMixin:
    """网页防篡改WPT 业务断言 Mixin。

    验证 WPT 实例的服务状态与虚拟机状态收敛。
    """

    def assert_wpt_status(self, name: str, service_status: str = "运行", vm_status: str = "运行", timeout: int = 300) -> dict:
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if service_status in svc and vm_status in vmst:
                    self.logger.info(f"WPT 实例 {name} 状态符合预期: 服务={svc}, 虚拟机={vmst}")
                    return row_data
            except Exception as e:
                self.logger.debug(f"读取 WPT 实例 {name} 状态失败: {e}")
            time.sleep(5)
        raise AssertionError(
            f"[StatusAssertion] WPT '{name}' | 状态未收敛 | "
