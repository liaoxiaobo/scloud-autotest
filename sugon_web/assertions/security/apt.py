...
class AptAssertionMixin:
    """攻击预警 APT 业务断言 Mixin。

    验证 APT 实例的服务状态与虚拟机状态收敛。
    ...
    def assert_apt_status(self, name: str, service_status: str = "运行", vm_status: str = "运行", timeout: int = 300):
        ...
        while time.time() - start_time < timeout:
            ...
            try:
                ...
                self.logger.info(f"APT 状态检查 #{iteration}: 服务状态='{svc}', 虚拟机状态='{vmst}', 期望=({service_status},{vm_status}), 耗时={int(time.time()-start_time)}s")
        ...
        raise AssertionError(
            f"[StatusAssertion] APT '{name}' | 状态未收敛 | "
