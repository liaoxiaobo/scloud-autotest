...
class RasAssertionMixin:
    ...
    def assert_ras_status(self, name: str, service_status: str = "运行", vm_status: str = "运行", timeout: int = 300) -> dict:
        ...
        while time.time() - start_time < timeout:
            ...
            try:
                ...
                self.logger.info(
                    f"RAS 状态检查 #{iteration}: 服务状态='{svc}', "
                    f"虚拟机状态='{vmst}', 期望=({service_status},{vm_status}), "
                ...
                if svc_match and vm_match:
                    self.logger.info(f"RAS 实例 {name} 状态符合预期: 服务={svc}, 虚拟机={vmst}")
                    return row_data
            except Exception as e:
                self.logger.warning(f"读取 RAS 实例 {name} 状态失败 (第{iteration}次): {e}")
            time.sleep(5)
        raise AssertionError(
            f"[StatusAssertion] RAS '{name}' | 状态未收敛 | "
