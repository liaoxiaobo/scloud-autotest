...
def validate_jobs(jobs):
    ...
    if not isinstance(jobs, list):
        raise ValueError(f"调度配置必须是数组或单个对象，当前类型: {type(jobs)}")
...
def build_run_jobs(dispatch_jobs, global_mark, default_host, default_stor):
    """构建最终执行任务列表（含默认兜底任务）。"""
...
def main():
    ...
    if args.use_env_dispatch:
        dispatch_jobs = validate_jobs(data)
    else:
        dispatch_jobs = load_from_env_yaml(args.input, args.hosts)

    run_jobs = build_run_jobs(dispatch_jobs, args.mark, args.host, args.stor)
