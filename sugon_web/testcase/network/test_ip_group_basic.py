...
class TestIpGroupBasic:
    ...
    def test_ip_group_create_delete(self, ip_group_page):
        ...
        with allure_step_log("步骤3: 删除IP地址组"):
            ip_group_page.ip_group_delete(group_name)

        with allure_step_log("步骤4: 验证IP地址组已删除"):
            ip_group_page.assert_deleted(group_name)

    @allure.title("验证搜索与重置IP地址组功能")
    def test_ip_group_search_reset(self, ip_group_page, ip_group):
        group_name = ip_group["name"]
        keyword = group_name[:-2]

        with allure_step_log(f"步骤1: 搜索IP地址组: {keyword}"):
            ip_group_page.ip_group_search(keyword)
            ip_group_page.assert_list_contain(keyword, exact_match=False)

        with allure_step_log("步骤2: 重置搜索条件"):
            ip_group_page.ip_group_search_reset()
            assert ip_group_page._input_search.input_value() == "", \
                f"重置后搜索框未清空，实际值: {ip_group_page._input_search.input_value()}"

        with allure_step_log("步骤3: 验证重置后列表恢复正常"):
            ip_group_page.assert_list_contain(group_name)

    @allure.title("验证编辑IP地址组功能")
    def test_ip_group_edit(self, ip_group_page, ip_group):
    ...
    def test_ip_group_detail_add_delete_ip(self, ip_group_page, ip_group):
        ...
        with allure_step_log(f"步骤2: 在详情页为 {group_name} 添加IP地址"):
            ip_group_page.ip_group_add_ip_addresses(group_name, extra_ips)
            ip_group_page.assert_popup_success()
        ...
        with allure_step_log("步骤4: 在详情页删除新增的IP地址"):
            ip_group_page.ip_group_delete_ip_addresses(group_name, extra_ips)
    ...
    def test_ip_group_detail_batch_delete_ip(self, ip_group_page, ip_group):
        group_name = ip_group["name"]
        ...
        with allure_step_log("步骤3: 在详情页批量删除新增的IP地址"):
            ip_group_page.ip_group_batch_delete_ip_addresses(group_name, extra_ips)
    ...
    def test_ip_group_batch_delete(self, ip_group_page):
        ...
        with allure_step_log("步骤2: 批量删除IP地址组"):
            ip_group_page.ip_group_delete(group_names)

        with allure_step_log("步骤3: 验证IP地址组已批量删除"):
            ip_group_page.assert_deleted(group_names)
