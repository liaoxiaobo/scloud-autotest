...
class TestVpnGatewaySSL:
    ...
    @pytest.mark.parametrize(
        "vm",
        [{"basic": {"count": 1}, "bind_mfip": True, "name_prefix": "vpn_"}],
        indirect=True,
    )
    @pytest.mark.parametrize(
        "eip",
    ...
    def test_vpn_gateway_ssl_vpc_basic_create(self, vm, vpc, eip, vpn_page):
        """测试SSL类型VPN网关新建功能。

        前置条件（由fixture准备）：
        1. VPC（vpn_前缀，CIDR 176.176.1.0/24）
        2. ECS（vpn_前缀，自动绑定MFIP）
        3. 3个FIP（public_net基础版）

        清理顺序：VPN网关（方法内）→ FIP（fixture teardown）→ ECS（fixture teardown）→ VPC（fixture teardown）
        """
        vpc_name = vpc["name"]
...
class TestVpnGatewayIpsec:
    ...
    def test_vpn_gateway_ipsec_vpc_basic_create(self, vm, vpc, eip, vpn_page):
        """测试IPSEC类型VPN网关新建功能。
