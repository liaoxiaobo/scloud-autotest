...
class TestSGBasic:
    ...
    def test_sg_search(self, vpc_page, sg):
        ...
        with allure_step_log(f"步骤1: 搜索安全组: {sg_name}"):
            ...
            # 使用模糊匹配断言列表中包含搜索关键字
            vpc_page.assert_list_contain(sg_name, exact_match=False)

        with allure_step_log(f"步骤2: 重置搜索条件"):
            vpc_page.sg_search_reset()
            assert vpc_page._input_search.input_value() == "", "重置后搜索输入框未被清空"

    @allure.title("安全组-修改名称和描述")
    def test_sg_edit(self, vpc_page, sg):
        sg_name = sg
        ...
        with allure_step_log(f"步骤1: 编辑安全组名称和描述: {sg_name} -> {new_name}"):
            ...
            vpc_page._btn_search.click()
            vpc_page.sg_edit(sg_name, new_name=new_name, new_desc=new_desc)
            vpc_page.assert_popup_success(f"{new_name}安全组修改成功")
            # 搜索后验证修改是否成功
            vpc_page.sg_search(new_name)
            vpc_page.assert_list_contain(new_name, exact_match=False)

        with allure_step_log(f"步骤2: 恢复安全组名称和描述: {new_name} -> {sg_name}"):
            vpc_page.sg_search_reset()
            vpc_page.sg_edit(new_name, new_name=sg_name, new_desc=orig_desc)
            vpc_page.assert_popup_success(f"{sg_name}安全组修改成功")
            vpc_page.sg_search(sg_name)
            vpc_page.assert_list_contain(sg_name, exact_match=False)
            vpc_page.sg_search_reset()

    @allure.title("安全组-克隆")
    def test_sg_clone(self, ecs_page, vpc_page, ssh_host, sg, vpc, eip):
        sg_name = sg
        ...
        with allure_step_log(f"步骤2: 克隆安全组 {sg_name} 为 {clone_name}"):
            vpc_page.sg_clone(sg_name, clone_name=clone_name, clone_desc=f"{clone_name}安全组")
            vpc_page.assert_popup_success("克隆安全组成功")
            vpc_page.assert_status(clone_name, status=f"{clone_name}安全组")

        with allure_step_log(f"步骤3: 验证克隆的{clone_name}安全组规则与原始安全组 {sg_name} 的规则完全相同"):
            # 获取原安全组规则
            orig_rules = vpc_page.sg_get_all_rules(sg_name)

            # 获取克隆安全组规则
            clone_rules = vpc_page.sg_get_all_rules(clone_name)
    ...
    def _test_sg_rule_delete(self, vpc_page, sg):
        ...
        with allure_step_log(f"步骤1: 在安全组 {sg_name} 中创建入口规则并删除"):
            ...
            # 删除创建的入口规则
            vpc_page.sg_rule_delete(sg_name, direction="入口")

    @allure.title("安全组规则-列表页快捷创建")
    def test_sg_rule_create_from_list(self, vpc_page, sg):
        sg_name = sg
        protocol_type = "HTTPS"
        direction = "入口"
        ...
        with allure_step_log(f"步骤2: 进入详情页，验证新创建的规则是否存在"):
            vpc_page.goto_sg_detail(sg_name)
            # 通过描述定位刚创建的规则行
            target_row = vpc_page.get_rows_by_text("列表页创建放行HTTPS").first
            # 验证各入参是否正确显示
    ...
    def test_sg_rule_create_scenarios(self, vpc_page, sg, scenario):
        ...
        with allure_step_log(f"步骤2: 验证规则"):
            vpc_page.goto_sg_detail(sg_name)
            target_row = vpc_page.get_rows_by_text(scenario["desc"]).first
            expect(target_row).to_be_visible()
            row_data = vpc_page.get_row_data_by_locator(target_row)
            # 验证方向
