...
class TestQosBasic:
    ...
    def test_qos_create_delete(self, qos_page):
        """验证网络QoS支持单条新建和删除"""
        ...
        with allure_step_log("步骤2: 验证网络QoS列表数据"):
            row_data = qos_page.get_row_data(qos_name)
        ...
        with allure_step_log("步骤3: 删除网络QoS"):
            qos_page.qos_delete(qos_name)

        with allure_step_log("步骤4: 验证网络QoS已删除"):
            qos_page.assert_deleted(qos_name)

    @allure.title("验证搜索与重置网络QoS功能")
    def test_qos_search_reset(self, qos_page, qos):
        """验证网络QoS搜索与重置"""
        qos_name = qos["name"]
        keyword = qos_name[:-2]

        with allure_step_log(f"步骤1: 搜索网络QoS: {keyword}"):
            qos_page.qos_search(keyword)
            qos_page.assert_list_contain(keyword, exact_match=False)

        with allure_step_log("步骤2: 重置搜索条件"):
            qos_page.qos_search_reset()
            assert qos_page._input_search.input_value() == "", \
                f"重置后搜索框未清空，实际值: {qos_page._input_search.input_value()}"

        with allure_step_log("步骤3: 验证重置后列表恢复正常"):
            qos_page.assert_list_contain(qos_name)

    @allure.title("验证编辑网络QoS功能")
    def test_qos_edit(self, qos_page, qos):
        """验证网络QoS支持修改名称、速率和描述"""
        old_name = qos["name"]
        ...
        with allure_step_log("步骤2: 验证修改结果"):
            row_data = qos_page.get_row_data(new_name)
    ...
    def test_qos_batch_delete(self, qos_page):
        """验证网络QoS支持批量删除"""
        ...
        with allure_step_log("步骤2: 批量删除网络QoS"):
            qos_page.qos_delete(qos_names)

        with allure_step_log("步骤3: 验证网络QoS已批量删除"):
            qos_page.assert_deleted(qos_names)
