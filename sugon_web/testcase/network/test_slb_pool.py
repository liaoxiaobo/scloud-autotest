...
class TestSlbPool:
    ...
    def test_lb_pool_add_resource(self, vpc_page, lb, lb_pool_candidate_vms, params):
        selected_vms = lb_pool_candidate_vms[:params["vm_count"]]
        vm_names = [vm["name"] for vm in selected_vms]
        expected_ports = vpc_page._resolve_expected_ports(params, selected_vms)
        added_vm_names = []
        ...
        with allure_step_log(f"步骤2: 为资源池 {lb['pool_name']} 新增资源 {vm_names}"):
            vpc_page.lb_pool_add_vm(vm_names=vm_names, ports=params["ports"])
        ...
        if added_vm_names:
            with allure_step_log(f"步骤4: 清理资源池成员 {added_vm_names}"):
                vpc_page.lb_pool_remove_vm(added_vm_names)
                vpc_page.assert_deleted(added_vm_names)
