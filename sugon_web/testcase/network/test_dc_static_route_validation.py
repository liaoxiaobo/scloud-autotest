...
@allure.story('静态路由生效性验证')
class TestDCStaticRouteValidation:

    @pytest.mark.parametrize(
        "vpc",
        [{"name": f"vpc-dc-route-{random_data(length=4)}", "subnet_name": f"subnet-dc-route-{random_data(length=4)}", "cidr": "13.34.34.0/24"}],
        indirect=True,
    )
    @pytest.mark.parametrize(
        "vm",
    ...
    @allure.title("云专线DC-非HA实例静态路由生效性验证")
    def test_static_route_validation(self, vm, vpc, dc_page, vpc_page, ssh_vm):
        """测试非HA实例静态路由生效性。

        前置条件（由脚本准备）：
        1. VPC+ECS（vm/vpc fixture自动创建）
        2. 非HA物理连接并审批通过（vlan=205）
        3. 虚拟网关关联VPC
        4. 虚拟接口（静态路由模式，远端子网123.12.0.0/24）
        ...
        try:
            ...
            with allure_step_log("步骤2: 创建自定义路由规则"):
                ...
                try:
                    ...
                    vpc_page.page.get_by_text(dc_name, exact=True).last.click()
            ...
            with allure_step_log("步骤4: SSH连接vm1验证静态路由生效"):
                ...
                # 浏览器端截图：路由表页面
                screenshot_path = f"screenshots/test_static_route_validation_ping_{int(time.time())}.png"
