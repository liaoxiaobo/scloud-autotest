    ...
    with allure_step_log(f"清理: 删除ER {er_name} 下目的网段 {dest_cidr} 的路由表规则"):
        for attempt in range(2):
            try:
                ...
                er_page.page.wait_for_timeout(500)
                ...
                er_page.close_dialog_if_exists()
                er_page.page.wait_for_timeout(1000)
...
def vpn_tunnel_er_noha_env(browser_context, config, ssh_host):
    ...
    3. 分配 3 个弹性公网 IP
    4. 创建 ER1 (vpn_er1, 非HA) + 添加VPC1为连接
    5. 创建 ER2 (vpn_er2, 非HA) + 添加VPC2为连接
    ...
    try:
        ...
        # 步骤6: 创建 ER1（非HA）
        with allure_step_log("前置: 创建企业路由器ER1（非HA）"):
            er_page.er_create(name=er1_name, cluster_name="Autotest", ha_enable=False)
        ...
        # 步骤7: 创建 ER2（非HA）
        with allure_step_log("前置: 创建企业路由器ER2（非HA）"):
            er_page.er_create(name=er2_name, cluster_name="Autotest", ha_enable=False)
            er_page.assert_popup_success(timeout=30)
...
@allure.story('VPN通道连接ER非HA生效性验证')
class TestVpnVpnConnectionErNohaEffectiveness:
    """VPN 通道连接 ER(非HA) 生效性验证（用例 420718）。

    单一场景，测试数据不可复用，前置资源在 class-scoped fixture 中创建，
    测试完成后统一清理。
    """

    @allure.title("VPN通道-连接ER(非HA)-生效性验证")
    def test_vpn_vpn_connection_er_noha_effectiveness(self, page, vpn_tunnel_er_noha_env, ssh_vm):
        """验证 VPN 通道连接 ER(非HA) 的生效性（用例 420718）。
        ...
        vpc1_name = vpn_tunnel_er_noha_env["vpc1_name"]
        vpc1_cidr = vpn_tunnel_er_noha_env["vpc1_cidr"]
        vpc2_name = vpn_tunnel_er_noha_env["vpc2_name"]
        vpc2_cidr = vpn_tunnel_er_noha_env["vpc2_cidr"]
        er1_name = vpn_tunnel_er_noha_env["er1_name"]
        er2_name = vpn_tunnel_er_noha_env["er2_name"]
        conn1_name = vpn_tunnel_er_noha_env["conn1_name"]
        conn2_name = vpn_tunnel_er_noha_env["conn2_name"]
        gw1_name = vpn_tunnel_er_noha_env["gw1_name"]
        gw2_name = vpn_tunnel_er_noha_env["gw2_name"]
        vm1_mfip = vpn_tunnel_er_noha_env["vm1_mfip"]
        vm2_mfip = vpn_tunnel_er_noha_env["vm2_mfip"]
        vm2_ip = vpn_tunnel_er_noha_env["vm2_ip"]
        vm1_ip = vpn_tunnel_er_noha_env["vm1_ip"]
