...
@allure.story('非HA-BGP生效性验证')
class TestDCBgpNonHaValidation:
    ...
    def test_bgp_non_ha_validation(self, vm, vpc, dc_page, vpc_page, ssh_vm):
        """测试BGP路由生效性。

        前置条件（由脚本准备）：
        1. VPC+ECS（vm/vpc fixture自动创建）
        2. 非HA物理连接并审批通过（vlan=705）
        ...
        try:
            ...
            with allure_step_log("步骤4: SSH连接vm1验证BGP路由生效"):
                ...
                logger.info(f"ping -c 4 10.9.9.9 输出:")
                ...
                # 浏览器端截图：路由表页面
                screenshot_path = f"screenshots/test_bgp_non_ha_validation_ping_{int(time.time())}.png"
