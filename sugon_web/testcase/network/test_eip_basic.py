...
class TestEipBasic:
    ...
    @allure.title("验证快速选择方式分配弹性公网IP功能")
    def test_eip_allocate_by_quick_select(self, vpc_page):
        """测试通过快速选择方式分配弹性公网IP"""

        with allure_step_log("步骤1: 通过快速选择方式分配1个弹性公网IP"):
        ...
        with allure_step_log("步骤2: 验证快速选择分配结果"):
            vpc_page.search(eip)
            vpc_page.assert_eip_list_contain(eip)
            row_text = vpc_page.get_row_by_name(eip).inner_text()
            assert eip in row_text, f"公网IP校验失败，期望包含: {eip}，实际行内容: {row_text}"
            vpc_page.btn_reset.click()
            vpc_page.wait_for_page_ready()

    @allure.title("验证手动输入方式分配弹性公网IP功能")
    def test_eip_allocate_by_manual_input(self, vpc_page):
        """测试通过手动输入方式分配弹性公网IP"""

        with allure_step_log("步骤1: 通过手动输入方式分配1个弹性公网IP"):
    ...
    @allure.title("验证释放弹性公网IP功能")
    @pytest.mark.parametrize("eip", [{"method": "手动输入"}], indirect=True)
    def test_eip_release(self, vpc_page, eip):
        """测试释放单个弹性公网IP"""

        with allure_step_log("步骤1: 验证待释放的弹性公网IP已存在"):
            vpc_page.search(eip)
            vpc_page.assert_eip_list_contain(eip)

        with allure_step_log("步骤2: 释放弹性公网IP"):
            vpc_page.eip_release(eip)

        with allure_step_log("步骤3: 验证弹性公网IP已释放"):
            vpc_page.assert_deleted(eip)

    @allure.title("验证弹性公网IP搜索重置功能")
    @pytest.mark.parametrize("eip", [{"method": "手动输入"}], indirect=True)
    def test_eip_search_reset(self, vpc_page, eip):
        """测试弹性公网IP搜索与重置"""

        with allure_step_log("步骤1: 搜索指定弹性公网IP"):
            vpc_page.search(eip)
            row_text = vpc_page.get_row_by_name(eip).inner_text()
    ...
    @allure.title("验证弹性公网IP批量释放功能")
    def test_eip_batch_release(self, vpc_page):
        """测试勾选列表页前两行弹性公网IP并批量释放"""

        with allure_step_log("步骤1: 获取列表页前两条弹性公网IP"):
            eips = vpc_page.get_eip_list()
            assert len(eips) >= 2, f"当前列表页弹性公网IP数量不足2条，实际: {len(eips)}"
            target_eips = eips[:2]

        with allure_step_log("步骤2: 批量释放前两条弹性公网IP"):
            vpc_page.eip_release(target_eips)

        with allure_step_log("步骤3: 验证前两条弹性公网IP已释放"):
