...
class TestERVPCConnectivityNonHA:

    @allure.title("企业路由器-非HA-连接VPC-生效性验证")
    def test_er_vpc_connectivity_non_ha(self, er_page, vpc_page, ecs_page, ops_page, ssh_vm, ssh_host):
        """测试非HA ER打通两个VPC后的连通性：创建VPC和VM、添加ER连接、配置路由、双向ping验证。"""
        ...
        er_name = f"er-non-ha-{random_data(length=4)}"
        ...
        try:
            ...
            with allure_step_log("步骤5: 创建非HA企业路由器"):
