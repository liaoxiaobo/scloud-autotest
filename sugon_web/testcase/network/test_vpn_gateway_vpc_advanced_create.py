...
@allure.story('VPN网关高级版VPC新建功能验证')
class TestVpnGatewaySSLAdvanced:
    """SSL类型VPN网关-高级版-连接VPC新建功能验证（用例437161）。"""
    ...
    @allure.title("VPN网关-SSL类型-连接VPC-高级版-新建功能验证")
    def test_vpn_gateway_ssl_vpc_advanced_create(self, vpc, vpn_page, clean_vpn_gateway):
        """测试SSL类型VPN网关-高级版-连接VPC新建功能。

        前置条件（由fixture准备）：
        1. VPC（vpn_前缀，CIDR 176.176.1.0/24）

        清理顺序：VPN网关（方法内）→ VPC（fixture teardown）
        """
        vpc_name = vpc["name"]
