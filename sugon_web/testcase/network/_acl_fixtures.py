...
from sugon_web.pages.network import AclPage
from sugon_web.utils.logger import allure_step_log, logger


def _do_clean_acl_rules(acl_page, acl_name, direction):
    """按方向清理指定 ACL 下的全部规则。
    ...
    with allure_step_log(f"Fixture清理: acl规则{direction} {acl_name}"):
        try:
            ...
            acl_page.goto_acl_detail(acl_name, tab_name=tab_name)
            while acl_page.get_by_role("row").filter(has=acl_page.get_by_text("删除", exact=True)).count() > 0:
                acl_page.acl_rule_delete(acl_name, direction=direction)
        except Exception as exc:
            logger.warning(f"清理ACL{direction}规则失败: {exc}")


@pytest.fixture(scope="function")
def clean_acl_inbound_rules(acl_page, acl):
    """在单用例结束后清理入方向规则。"""
    yield
    _do_clean_acl_rules(acl_page, acl, "入方向")
...
def clean_acl_outbound_rules(browser_context, config, acl):
    ...
    acl_page = AclPage(page)
    try:
        _do_clean_acl_rules(acl_page, acl, "出方向")
    finally:
        page.close()
