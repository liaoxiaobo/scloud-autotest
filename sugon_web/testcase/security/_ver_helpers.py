...
def create_ver_instance(page, ver_page, name: str) -> dict[str, Any]:
    ...
    for attempt in range(1, 4):
        try:
            ...
            ver_page.ver_create(name=actual_name)
            ver_page.assert_ver_status(actual_name, service_status="运行", vm_status="运行", timeout=1200)
            name = actual_name
            break
        except AssertionError as e:
            if "创建失败" in str(e) and attempt < 3:
                logger.warning(f"VER 实例创建失败，将重新创建: {e}")
                continue
            raise
