...
@allure.story('USM实例-热迁移-系统分配')
class TestUsmHotMigrationSystem:

    @allure.title("USM-热迁移-系统分配")
    @skip_if_nodes_less_than(2)
    def test_usm_hot_migration_system(self, usm_instance, usm_page, ssh_host):
        ...
        with allure_step_log("步骤5: 等待迁移完成并从页面读取实际的目标物理机"):
            ...
            logger.info(
                f"系统分配结果: 源物理机={source_host} → 目标物理机={target_host}"
            )
