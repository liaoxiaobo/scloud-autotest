...
class TestBmsRebuild:
    ...
    def test_bms_rebuild(self, bms_page, bms_instance_name):
        ...
        # 步骤3：重建前解绑公网IP，避免FIP绑定状态影响重建流程
        with allure_step_log("步骤3: 重建前解绑公网IP"):
            bms_page._goto_submenu_safe("裸金属实例")
            bms_page.bms_search(instance_name)
            ...
            if "公网:" in row_text:
                ...
                row = bms_page._get_row_by_name(instance_name)
                row_text = row.text_content() if row else ""
                assert "公网:" not in row_text, f"实例 '{instance_name}' 公网IP解绑后列表仍显示公网IP"
                logger.info(f"实例 '{instance_name}' 重建前公网IP已解绑")
            else:
                logger.info(f"实例 '{instance_name}' 未绑定公网IP，跳过解绑")
        ...
        # 步骤6：等待重建完成，状态恢复为运行中
        with allure_step_log("步骤6: 等待重建完成"):
            rebuild_ok = bms_page.bms_instance_wait_for_status(
                instance_name, "运行中", timeout=BMS_REBUILD_TIMEOUT
            )
            if not rebuild_ok:
                bms_page._goto_submenu_safe("裸金属实例")
                bms_page.bms_search(instance_name)
                row_data = bms_page.get_row_data(instance_name) or {}
                status = row_data.get("状态", "")
                logger.warning(f"重建超时后实例最终行数据: {row_data}")
                assert rebuild_ok, (
                    f"实例未在{BMS_REBUILD_TIMEOUT // 60}分钟内恢复为运行中状态，"
                    f"最终状态: '{status or '未获取到'}'"
                )

        # 步骤7：验证列表状态
        with allure_step_log("步骤7: 验证列表状态"):
            bms_page._goto_submenu_safe("裸金属实例")
            bms_page.bms_search(instance_name)
            row_data = bms_page.get_row_data(instance_name)
            assert row_data is not None, f"未找到实例 '{instance_name}'"
