...
class TestBmsStart:
    ...
    def test_bms_start(self, bms_page, bms_instance_name):
        """验证裸金属实例开机功能正常。"""
        ...
        try:
            ...
            with allure_step_log("步骤4: 验证列表中状态已更新为运行中"):
                ...
                assert start_ok, "实例未在5分钟内变为运行中状态"
            ...
            if prepared_shutdown:
                try:
                    bms_page._goto_submenu_safe("裸金属实例")
                    bms_page.bms_search(instance_name)
                    row_data = bms_page.get_row_data(instance_name)
                    status = row_data.get("状态", "") if row_data else ""
                    if "运行中" not in status:
                        bms_page.bms_instance_start(instance_name)
                        bms_page.bms_instance_wait_for_status(instance_name, "运行中", timeout=300)
                except Exception as e:
                    logger.warning(f"开机用例清理恢复失败: {e}")
