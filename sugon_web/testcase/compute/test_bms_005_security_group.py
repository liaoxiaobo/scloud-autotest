...
class TestBmsSecurityGroup:
    ...
    def test_bms_set_security_group(self, bms_page, sg, bms_instance_name):
        ...
        try:
            ...
            if changed and original_sg:
                with allure_step_log("清理: 恢复原始安全组"):
                    try:
                        bms_page._goto_submenu_safe("裸金属实例")
                        bms_page.bms_search(instance_name)
                        ...
                        logger.info(f"安全组已恢复为 '{original_sg}'")
                    except Exception as e:
                        logger.warning(f"安全组恢复失败: {e}")
