...
def _execute_full_backup_and_collect_data(backup_page: BackUpPage, backup_task: TaskInfo) -> TaskInfo:
    ...
    这是一个场景 helper，而不是 fixture。它将“执行一次全量备份”这一较重的
    业务动作保留在测试编排层附近，同时把重复实现集中起来复用。

    Args:
        backup_page: 备份页面对象，用于触发任务执行并查询结果。
        backup_task: ``backup_task`` fixture 返回的任务元数据。

    Returns:
        一个字典，包含原始任务元数据，以及收集到的备份数据和源虚机校验字段。

    Example:
        backup_data = _execute_full_backup_and_collect_data(backup_page, backup_task)
        assert backup_data["backup_data"]
    """
