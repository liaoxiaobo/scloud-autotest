...
class TestAclIP:
    ...
    def test_ip_allow(self, iam_page, iam_shared_user):
        disp = iam_shared_user["display_name"]
        u = iam_shared_user["name"]
        p = iam_shared_user["password"]
        with allure_step_log("步骤1: 清空IP限制"):
            iam_page.iam_set_access_control(disp, ip="")
...
class TestAclDate:
    ...
    def test_date_range(self, iam_page, iam_shared_user):
        disp = iam_shared_user["display_name"]
        u = iam_shared_user["name"]
        p = iam_shared_user["password"]
        ...
        with allure_step_log(f"步骤1: 设置日期 {today}~{future}"):
            iam_page.iam_set_access_control(disp, start_date=today, end_date=future)
        with allure_step_log("步骤2: 验证登录成功"):
            assert verify_login(iam_page.page, u, p, expect_success=True)

    @allure.title("IAM-用户管理-访问控制-过期日期范围")
    def test_date_past(self, iam_page, iam_shared_user):
        disp = iam_shared_user["display_name"]
        u = iam_shared_user["name"]
        p = iam_shared_user["password"]
        ...
        with allure_step_log(f"步骤1: 设置日期 {past}~{yesterday}"):
            iam_page.iam_set_access_control(disp, start_date=past, end_date=yesterday)
        with allure_step_log("步骤2: 验证登录失败"):
            assert verify_login(iam_page.page, u, p, expect_success=False)

    @allure.title("IAM-用户管理-访问控制-未来日期范围")
    def test_date_future(self, iam_page, iam_shared_user):
        disp = iam_shared_user["display_name"]
        u = iam_shared_user["name"]
        p = iam_shared_user["password"]
        ...
        with allure_step_log(f"步骤1: 设置日期 {tomorrow}~{day_after}"):
            iam_page.iam_set_access_control(disp, start_date=tomorrow, end_date=day_after)
        with allure_step_log("步骤2: 验证登录失败"):
            assert verify_login(iam_page.page, u, p, expect_success=False)

    @allure.title("IAM-用户管理-访问控制-仅当天日期")
    def test_date_today(self, iam_page, iam_shared_user):
        disp = iam_shared_user["display_name"]
        u = iam_shared_user["name"]
        p = iam_shared_user["password"]
        today = date.today().strftime("%Y-%m-%d")
        with allure_step_log(f"步骤1: 设置日期仅 {today}"):
            iam_page.iam_set_access_control(disp, start_date=today, end_date=today)
        with allure_step_log("步骤2: 验证登录成功"):
            assert verify_login(iam_page.page, u, p, expect_success=True)
...
class TestAclTime:
    ...
    def test_time_match(self, iam_page, iam_shared_user):
        disp = iam_shared_user["display_name"]
        u = iam_shared_user["name"]
        p = iam_shared_user["password"]
        now = datetime.now()
    ...
    def test_time_mismatch(self, iam_page, iam_shared_user):
        disp = iam_shared_user["display_name"]
        u = iam_shared_user["name"]
        p = iam_shared_user["password"]
        now = datetime.now()
...
class TestAclCombined:
    ...
    def test_combined_deny_ip(self, iam_page, iam_shared_user):
        disp = iam_shared_user["display_name"]
        u = iam_shared_user["name"]
        p = iam_shared_user["password"]
        today = date.today().strftime("%Y-%m-%d")
    ...
    def test_combined_deny_time(self, iam_page, iam_shared_user):
        disp = iam_shared_user["display_name"]
        u = iam_shared_user["name"]
        p = iam_shared_user["password"]
        today = date.today().strftime("%Y-%m-%d")
    ...
    def test_combined_deny_date(self, iam_page, iam_shared_user):
        disp = iam_shared_user["display_name"]
        u = iam_shared_user["name"]
        p = iam_shared_user["password"]
        yesterday = (date.today() - timedelta(days=1)).strftime("%Y-%m-%d")
    ...
    def test_combined_allow(self, iam_page, iam_shared_user):
        disp = iam_shared_user["display_name"]
        u = iam_shared_user["name"]
        p = iam_shared_user["password"]
        today = date.today().strftime("%Y-%m-%d")
