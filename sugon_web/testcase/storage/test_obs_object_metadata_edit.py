...
@allure.story('元数据-编辑元数据并验证元数据生效')
class TestOBSObjectMetadataEdit:

    @pytest.mark.parametrize("bucket", [{}], indirect=True)
    @allure.title("对象存储-编辑元数据并验证元数据生效")
    def test_obs_object_metadata_edit_validation(self, obs_page, bucket, page):
        """验证对象元数据可编辑，且编辑后通过HTTP HEAD请求验证服务端生效。"""
        ...
        # ------------------ 步骤5：编辑元数据 ------------------
        with allure_step_log("步骤5: 编辑元数据值"):
            obs_page.obs_object_metadata_edit(metadata_key, edited_value)

        # ------------------ 步骤6：验证元数据编辑后UI显示 ------------------
        with allure_step_log("步骤6: 验证编辑后元数据值"):
            edited_actual = obs_page.obs_object_metadata_get_value(metadata_key)
            assert edited_actual == edited_value, (
                f"元数据编辑后值不匹配 | 期望: {edited_value} | 实际: {edited_actual}"
            )
        ...
        with allure_step_log("步骤7: HEAD请求验证元数据服务端生效"):
            ...
            assert header_value == edited_value, (
                f"元数据服务端值不匹配 | 期望: {edited_value} | 实际: {header_value}"
            )
