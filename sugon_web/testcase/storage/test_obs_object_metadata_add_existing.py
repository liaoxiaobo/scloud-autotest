...
@allure.story('元数据-存量对象增加元数据并验证生效性')
class TestOBSObjectMetadataAddExisting:

    @pytest.mark.parametrize("bucket", [{}], indirect=True)
    @allure.title("对象存储-存量对象增加元数据并验证生效性")
    def test_obs_object_metadata_add_existing_validation(self, obs_page, bucket, page):
        """验证存量对象可添加元数据，且添加后通过HTTP HEAD请求验证服务端生效。"""
        ...
        # ------------------ 步骤1：创建个人凭证 ------------------
        with allure_step_log("步骤1: 检查并清理已有凭证，创建新凭证并记录ak/sk"):
            obs_page.goto_submenu("个人凭证")
            obs_page.page.wait_for_timeout(2000)
            # 检查并清理已有凭证（每个项目至多2个访问密钥）
            existing_rows = obs_page.page.locator(".el-table__row")
            if existing_rows.count() > 0:
                for i in range(min(existing_rows.count(), 2)):
                    ak_cell = existing_rows.nth(i).locator("td").nth(1)
                    existing_ak = ak_cell.inner_text().strip()
                    if existing_ak:
                        obs_page.obs_credential_delete(existing_ak)
                        obs_page.page.wait_for_timeout(2000)
            obs_page.page.wait_for_timeout(2000)
        ...
        # ------------------ 步骤3：上传对象（不配置元数据） ------------------
        with allure_step_log("步骤3: 上传对象"):
            obs_page.obs_object_tab_click()
            obs_page.obs_object_upload(test_file_path)

        # ------------------ 步骤4：验证对象上传成功及元数据初始状态 ------------------
        with allure_step_log("步骤4: 验证元数据初始状态为空"):
            obs_page.goto_submenu("桶列表")
            obs_page.page.wait_for_timeout(2000)
            obs_page.obs_bucket_enter_detail(bucket_name)
            obs_page.obs_object_tab_click()
            ...
            assert actual_value is None, (
                f"元数据初始值应为空 | 实际: {actual_value}"
            )

        # ------------------ 步骤5：为存量对象添加元数据 ------------------
        with allure_step_log("步骤5: 添加元数据"):
            obs_page.obs_object_metadata_add(metadata_key, metadata_value)

        # ------------------ 步骤6：验证元数据添加后UI显示 ------------------
        with allure_step_log("步骤6: 验证添加后元数据值"):
            added_actual = obs_page.obs_object_metadata_get_value(metadata_key)
            assert added_actual == metadata_value, (
                f"元数据添加后值不匹配 | 期望: {metadata_value} | 实际: {added_actual}"
            )

        # ------------------ 步骤7：通过HTTP HEAD请求验证元数据服务端生效 ------------------
        with allure_step_log("步骤7: HEAD请求验证元数据服务端生效"):
            # 解析host
        ...
        # ------------------ 清理：删除个人凭证 ------------------
        with allure_step_log("清理: 删除个人凭证"):
