...
//         string(name: 'BRANCH', defaultValue: 'develop', description: '请输入正确Git分支名（如main、develop)', trim: true)
  ...
  stor: xstor
  parallel_count: 3''', description: 'JSON 或 YAML 格式环境调度配置，优先级高于 env.yaml。支持默认执行环境条目（无 modules/services/mark）和具体 dispatch 任务列表。默认执行环境用于兜底未分配的模块/服务。每项包含 host、modules/services/mark、stor，可选 parallel_count。YAML 示例：\n# 默认执行环境（兜底）\n- host: "172.22.1.190"\n  stor: ceph\n  parallel_count: 3\n# 具体模块/服务调度\n- host: "172.22.3.140"\n  modules: [compute]\n  stor: xbd\n  parallel_count: 4\n- host: "172.22.1.190"\n  services: [evs, vpc]\n  stor: xstor\n  parallel_count: 2')
                    ...
                    jobs.each { job ->
                        ...
                        branches["test-${currentJob.label}"] = {
                            docker.image("playwright-sugon:${IMAGE_TAG}").inside() {
                                ...
                                def pytestCommand = "pytest --headless=true --host=${currentJob.host} --stor=${currentJob.stor} --user-role=${params.USER_ROLE} --env-label=${currentJob.label} \"${workspaceDir}/sugon_web/testcase/\" -n ${parallelCount} --dist=loadscope"
