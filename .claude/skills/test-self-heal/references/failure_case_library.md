...
> **维护约定（重要）**：
> - 本库是 test-self-heal 的**自包含资产**，阶段三（`phase3-self-heal-repair`）诊断时按需 `Read` 本文件，命中已收录模式即优先套用其根因方向与修复建议，避免从零试错。**本库完全自包含，执行时无需去读任何其它 skill 的文件。**
> - **后期新增案例只需按下方统一格式（模式名 / 现象 / 根因 / 关键证据链 / 修复方向 / 排查口诀）追加到本文件**，无需改动任何 phase 文件或 SKILL.md——这是案例库做成独立文件而非内联的目的：维护成本最低、不污染 agent 正文、不拖累子智能体启动上下文。
