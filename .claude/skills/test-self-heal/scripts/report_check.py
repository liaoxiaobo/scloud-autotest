#!/usr/bin/env python3
"""report_check.py — test-self-heal 运行报告「锚点完整性」校验黑盒脚本。

用途：各 phase 文件末尾「阶段完成时立即输出」要求把规定内容**双写**到运行报告
`skill_runs/test-self-heal/test-self-heal_{任务标识}_*.md`。但该输出发生在
...
用法示例：
    ...
    python report_check.py skill_runs/test-self-heal/test-self-heal_xxx_YYYYMMDD_HHMM.md --phase 5
    # 总收尾前校验全部阶段 + 总收尾锚点齐全
    python report_check.py skill_runs/test-self-heal/test-self-heal_xxx_YYYYMMDD_HHMM.md --phase final
    # 也可只校验到某个中间阶段
...
REPORT_ANCHORS = {
    "1": [
        ("## 阶段一：", "阶段一完成块标题（含完成时间）"),
        ("范围基线", "阶段一范围基线区块（自愈范围/模块→前端目录映射/版本基线）"),
        ("失败清单", "阶段一失败清单表（每个失败用例一条完整证据）"),
    ],
    "2": [
        ("## 阶段二：", "阶段二完成块标题（含完成时间）"),
        ("分诊", "阶段二 4 分类根因分诊表"),
        ("疑似产品缺陷清单", "阶段二「疑似产品缺陷清单（需人工确认）」区块（无缺陷也须出、写明'本次未发现'，机械保证产品缺陷不被漏报）"),
    ],
    "3": [
        ("## 阶段三：", "阶段三完成块标题（含完成时间）"),
        ("完成清单", "阶段三完成清单表"),
        ("问题清单", "阶段三修复循环问题清单（含根因列/修复方案/采用的前端对齐手段）"),
        ("Jenkins 对齐执行结果", "阶段三 Jenkins 对齐执行结果行（保证本地通过=Jenkins通过）"),
    ],
    "4": [
        ("## 阶段四：", "阶段四完成块标题（含完成时间）"),
        ("完成清单", "阶段四回归结果/完成清单表"),
        ("回归结果概览", "阶段四回归结果概览（可自愈集合总数/已通过数/未通过数/待修集是否清空/判定去向，供编排层回退判定与阶段五前置守卫用）"),
    ],
    "5": [
        ("## 阶段五：", "阶段五完成块标题（含完成时间）"),
        ("完成清单", "4.2 完成清单表"),
        ...
        ("第二部分：修复经验沉淀总结", "4.4 第二部分修复经验沉淀表标题"),
    ],
    "final": [
        ("## 完成后输出", "全局总结标题"),
        ("总体结论", "总结 1. 总体结论"),
        ("执行概览", "总结 2. 执行概览表"),
        ("风险与后续待办", "总结 3. 风险与后续待办（产品缺陷置顶醒目）"),
        ("产物位置索引", "总结 4. 产物位置索引"),
...
def _validate_report(report_path, phase):
    ...
    p = Path(report_path)
    if not p.exists():
        return [f"运行报告文件不存在：{p}（各阶段须双写到 skill_runs/test-self-heal/ 下的同一文件）"]
    text = _read(p)
    if text is None:
...
def _parse_args(argv):
    parser = argparse.ArgumentParser(
        description="test-self-heal 运行报告锚点校验：校验各阶段「立即输出」要求的章节是否齐全（对抗上下文压缩导致的漏写/写简）。",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    parser.add_argument("report", help="本次运行报告 md 路径（skill_runs/test-self-heal/ 下）")
