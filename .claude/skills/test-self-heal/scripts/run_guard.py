...
=== 计数单位的明确定义（务必读懂，避免理解错）===
    本脚本的计数键 = 你传给 pytest 的"测试目标"：
      · `pytest 文件.py -k test_xxx`  → 键 = 该【测试方法 test_xxx】（推荐：阶段三调试单个失败用例就这么跑）
      · `pytest 文件.py`（不带 -k）   → 键 = 【整个 .py 文件】（首次整文件跑 / 最后整文件验证）
    这里的"单个用例" = 测试脚本里的【一个 `def test_` 方法】（用 -k 单独跑时）。
    所以："单个用例最多跑 N 次" = "每一个失败的 `def test_` 方法最多跑 N 次 pytest"。
    它【不是】指整个 .py 脚本（一个 .py 往往含多个 def test_ 方法），也【不是】指一次 pytest 命令。
...
机械控制（全部确定性，不含 LLM 判断，口径极简：1 次 pytest = 1 次）：
    ...
    2) 硬熔断（满足任一即退出码 3 拒绝执行）：
         ...
         - 全局累计 >= --cap-global（默认随失败用例数缩放 = max(60, 失败用例数 × 10)）
...
用法（阶段三所有 pytest 必须经本脚本跑，禁止裸跑 pytest）：
    ...
    python .claude/skills/test-self-heal/scripts/run_guard.py --reset --task <任务标识> --cases <失败用例数>
    # 阶段四/五【回退重入】阶段三时，用 --reset --reentry（不清零累计全局、只给本用例新预算 + 小额回退预算）：
    python .claude/skills/test-self-heal/scripts/run_guard.py --reset --reentry --task <任务标识> --cases <失败用例数>
    # 之后每次跑 pytest 都经本脚本（-- 之后原样就是平时的 pytest 命令与参数）：
    python .claude/skills/test-self-heal/scripts/run_guard.py --task <任务标识> -- \
...
DEFAULT_CAP_FILE = 40          # 单测试目标（= 一个 def test_ 方法）次数上限（沿革：2026-06-19 由 20→30；2026-06-21 由 30→40：弱模型修复准确度低、复杂用例 setup 链长，给更多修复空间；有"连续相同失败冻结"兜底，提高上限不会退回无限续命）
GLOBAL_PER_CASE = 10           # 全局上限按"每个场景 N 次"加总缩放：失败用例数 × 该值（沿革：2026-06-17 由 30→8；2026-06-21 由 8→10：配合单用例上限提高，给整体更宽裕额度）
...
def _state_path(task, explicit):
    ...
    return _project_root() / "skill_runs" / "test-self-heal" / f"_runguard_{safe}.json"
...
def _sentinel_path():
    return _project_root() / "skill_runs" / "test-self-heal" / "_guard_active.json"
...
def _latest_report(task):
    ...
    d = _project_root() / "skill_runs" / "test-self-heal"
    if not d.is_dir():
        return None
    safe = re.sub(r"[^A-Za-z0-9_.-]", "_", task or "default")
    for pat in (f"test-self-heal_{task}_*.md", f"test-self-heal_{safe}_*.md"):
...
def main(argv):
    ...
    ap.add_argument("--cap-file", type=int, default=DEFAULT_CAP_FILE,
                    help=f"单测试目标(= 一个 def test_ 方法)次数上限（默认 {DEFAULT_CAP_FILE}）")
    ap.add_argument("--cap-global", type=int, default=DEFAULT_CAP_GLOBAL,
                    help=f"全局次数上限下限（默认 {DEFAULT_CAP_GLOBAL}）；实际取 max(该值, 失败用例数×{GLOBAL_PER_CASE})")
    ...
    ap.add_argument("--cases", type=int, default=None,
                    help=f"本次测试失败用例数（def test_ 方法数）；用于把全局上限缩放为 max(cap-global, 失败用例数×{GLOBAL_PER_CASE})，--reset 时传一次")
    ...
    if args.reset:
        # 全局上限随失败用例数缩放 = max(下限, 失败用例数 × 每个场景 10 次)
        ...
        if args.reentry:
            ...
            if (not args.force) and prev_global > 0 and age is not None and age < REACTIVATION_GUARD_SECS:
                ...
                print("[run_guard] → 若你是阶段三续跑（含压缩后续跑）：无需任何 reset，直接继续 "
                      "`python .claude/skills/test-self-heal/scripts/run_guard.py --task <标识> -- pytest ...` 即可。")
            ...
            print(f"[run_guard] 次数上限：单用例(= 一个 def test_ 方法)={args.cap_file} 次、"
                  f"全局={eff_global} 次（失败用例数={args.cases or '未提供'}，公式 max({args.cap_global}, 失败用例数×{GLOBAL_PER_CASE})）；"
