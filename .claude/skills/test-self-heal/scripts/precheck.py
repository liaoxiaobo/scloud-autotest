#!/usr/bin/env python3
"""precheck.py — test-self-heal 阶段二/三「静态门禁」黑盒脚本。
...
def _check_recon_gate(test_files, explicit_page_files, recon_dir, since_epoch):
    ...
    return [
        ...
        f"（如弹窗 vs 内联表单）。请执行："
        f"python .claude/skills/test-self-heal/scripts/recon_page.py --service \"<服务>\" [--submenu \"<子菜单>\"] "
...
def _parse_args(argv):
    parser = argparse.ArgumentParser(
        description="test-self-heal 静态门禁：首次执行 pytest 前对 test_*.py 做客观机械校验。",
    ...
    parser.add_argument("--expected-tests", type=int, default=None,
                        help="可选：期望的 def test_ 方法总数（新写脚本时应等于需求场景数）；"
                             "自愈维护存量文件时通常【不传】此参数——存量文件方法数是既有的、与本次失败用例数无关，"
                             "传了会因数量不符而误报。不一致则报错")
...
def main(argv):
    ...
    if args.expected_tests is not None:
        if total_cases != args.expected_tests:
            all_violations.append(
                ...
                f"严禁把多个场景塞进同一个测试方法；仅『参数不同的高重复场景』才用 @pytest.mark.parametrize 实现。"
                f"（注：自愈维护存量文件时通常不应传 --expected-tests，若此项误报请去掉该参数重跑）"
            )
    else:
        count_skipped = True
    ...
    if count_skipped:
        print("[INFO] 未提供 --expected-tests，已【跳过】数量对齐门禁。"
              "（自愈维护存量文件属正常：文件既有方法数与本次失败用例数无关，本就不该做数量对齐。）")
    elif indeterminate:
