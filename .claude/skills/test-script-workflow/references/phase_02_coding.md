...
>
> **落笔前就近重读 + 写后校验（强制）**：生成本阶段输出前，先重新 `Read` 本文件「## 阶段二完成时立即输出」整节，按其确切要求逐项写全（状态标题 + 完成清单表）；写完后运行 `python .claude/skills/test-script-workflow/scripts/report_check.py <本次运行报告 md 路径> --phase 2`，退出码非 0 则按提示补写后重跑，通过方可进入下一阶段。
