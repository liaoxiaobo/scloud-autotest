...
4. `sugon_web/refrence/module_index.yaml`，这个关联文件是前端工程代码的模块索引与搜索策略定义文档，其中"模块匹配策略"章节定义了被测模块与前端目录的映射规则，用于定位本次测试需求对应的被测模块的前端工程代码路径；"前端代码搜索工具约束"章节规定了查阅前端代码时的搜索策略与可用工具限制。读取该文件后，须严格遵照其中定义的匹配策略和搜索约束，定位并读取匹配到的前端工程代码：
...
> 第一部分编写完脚本后，对照阶段一产出的需求 MD、第一部分引用的编码规范（`sugon_web/case_specs/prompts/test_case_codegen_prompt.md`）**以及对应前端工程代码**，按以下检查项逐项检查，发现遗漏或偏差直接修改，循环直至完全对齐。
